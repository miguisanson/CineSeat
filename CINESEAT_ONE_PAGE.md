...
Reviews contain no bundled posts. A signed-in account can create one rating/comment per movie, concert, or seminar only when it owns a confirmed ticket and the scheduled start time plus the locally stored duration has passed. The same account can edit or delete its review. Reviews are stored in Documents as `reviews.json`. Developer Mode can locally simulate eligibility for presentation testing without weakening the normal rule.
